...
struct Node {
  ...
  explicit Node(size_t k)
      : frame_id_(0),
        ...
        evitable(true),
        k_(k),
        iter(std::list<std::shared_ptr<Node>>::iterator()),
        timestamp_list_() {}
...
class LRUKReplacer {
 public:
  ...
  bool remove(frame_id_t frame_id, std::unordered_map<frame_id_t, std::shared_ptr<Node>> &access_,
 ...
 private:
  ...
  size_t k_;
  std::mutex latch_;
  std::unordered_map<frame_id_t, std::shared_ptr<Node>> history_access, cache_access;
